...
| **2. Backend-team tickets** | File/track, in parallel with 1a/1b, not after: (a) the webhook-dedup fix (`idea.md` Finding 4 — a pre-existing production bug, independent of this project's timeline), (b) the `eval-` test tenant + Square Sandbox credentials (`idea.md` Finding 9). | None — these can and should start immediately; everything in Phase 3 is blocked waiting on (b). |
